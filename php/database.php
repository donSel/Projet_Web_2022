<?php
    ...
    function getOrganizerPlayerMatchIdTitle($db, $mail){
        $request = 'SELECT match_id, title FROM match WHERE organizer_id = :mail_organizer OR (SELECT mail FROM player WHERE mail = :mail_player)';
        $statement = $db->prepare($request);
    ...
    function getPLayersOfAMatch($db, $match_id){
        $request = "SELECT t.mail, p.last_name, p.photo_url FROM p player, t play WHERE t.match_id = :match_id AND t.mail = p.mail AND t.is_registered = 'true'";
        $statement = $db->prepare($request);
    ...
    function getInfoEvent($db, $match_id){
        $request = "SELECT m.id_match, m.title, m.match_decription, o.lastname, o.photo_url FROM m match, o player WHERE m.match_id = :match_id AND o.mail = m.organizer_id";
        $statement = $db->prepare($request);
        $statement->bindParam(':match_id', $match_id);
        $statement->execute();
        $data = $statement->fetchAll(PDO::FETCH_ASSOC); 
