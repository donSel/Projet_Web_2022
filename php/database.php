<?php
    ...
    function getAllProfilEvents($db, $idMatch){
        /*
         SELECT m.match_id, m.title, m.is_finished, r.best_player, p.photo_url, p.last_name, t.role, m.hour, m.duration, v.town, m.address, r.score_match, r.winner
                    FROM match m, player p, match_result r, play t, town v
                    WHERE m.match_id=24 AND m.match_id=t.match_id AND m.match_id=r.match_id AND r.best_player=p.mail AND m.town_id=v.town_id
         */
